...
def _make_three_unique_probes(marker: str) -> List[Dict[str, str]]:
    """
    Velocity probes (safe, non-RCE):
      1) #set($x=A*B)$x -> number
      2) #set($s="a" + "b")$s -> ab
      3) #set($t="abc")$t.toUpperCase() -> ABC
    """
    ...
    expected1 = str(a * b)
    expr1 = f"#set($x={a}*{b})$x"
    probes.append({"type": "arith_set", "payload": f"{marker}{expr1}", "expr": expr1, "expected": expected1})

    s1 = _rand_token(6)
    s2 = _rand_token(6)
    expected2 = f"{s1}{s2}"
    expr2 = f'#set($s="{s1}" + "{s2}")$s'
    probes.append({"type": "concat_set", "payload": f"{marker}{expr2}", "expr": expr2, "expected": expected2})

    s3 = _rand_token(10)
    expected3 = s3.upper()
    expr3 = f'#set($t="{s3}")$t.toUpperCase()'
    probes.append({"type": "upper_method", "payload": f"{marker}{expr3}", "expr": expr3, "expected": expected3})
