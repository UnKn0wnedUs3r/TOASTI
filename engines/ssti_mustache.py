...
def _make_three_unique_probes(marker: str) -> List[Dict[str, str]]:
    """
    Mustache/Handlebars-like probes (safe):
      1) {{#if 1}}TOKEN{{/if}} -> TOKEN
      2) {{#unless false}}TOKEN{{/unless}} -> TOKEN
      3) {{#with 'abc'}}{{this}}{{/with}} -> abc
    """
    probes: List[Dict[str, str]] = []

    tok1 = _rand_token(12)
    expr1 = f"{{{{#if 1}}}}{tok1}{{{{/if}}}}"
    probes.append({"type": "if_truthy", "payload": f"{marker}{expr1}", "expr": expr1, "expected": tok1})

    tok2 = _rand_token(12)
    expr2 = f"{{{{#unless false}}}}{tok2}{{{{/unless}}}}"
    probes.append({"type": "unless_false", "payload": f"{marker}{expr2}", "expr": expr2, "expected": tok2})

    s3 = _rand_token(10)
    expr3 = f"{{{{#with '{s3}'}}}}{{{{this}}}}{{{{/with}}}}"
    probes.append({"type": "with_string", "payload": f"{marker}{expr3}", "expr": expr3, "expected": s3})

    return probes


def mustache_ssti_scan(client: HTTPClient, targets: List[Target]) -> List[Dict[str, Any]]:
