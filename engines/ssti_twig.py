...
def _rand_marker() -> str:
    return "toasti_twig_" + _rand_token(10)
...
def _make_three_unique_probes(marker: str) -> List[Dict[str, str]]:
    """
    Twig syntax probes (safe, non-RCE):
      1) arithmetic: {{A*B}} -> number
      2) concat: {{'a' ~ 'b'}} -> ab
      3) filter: {{'abc'|upper}} -> ABC
    """
    ...
    expr3 = f"{{{{'{s3}'|upper}}}}"
    expected3 = s3.upper()
    probes.append({"type": "upper", "payload": f"{marker}{expr3}", "expr": expr3, "expected": expected3})

    return probes


def twig_ssti_scan(client: HTTPClient, targets: List[Target]) -> List[Dict[str, Any]]:
